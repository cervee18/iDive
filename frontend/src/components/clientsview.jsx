...
import { 
  Search, Save, Calendar, MapPin, Home, 
  Users, Plus, Trash2, X, Edit2, Anchor, ExternalLink
} from 'lucide-react';
...
const ClientsView = () => {
  ...
  const [profileForm, setProfileForm] = useState({
    first_name: '', last_name: '', email: '', certification_level: '', total_dives: 0
  });
  ...
  const fetchVisitsForClient = async (clientId) => {
    ...
    if (visitIds.length > 0) {
      ...
      if (error) console.log('Error fetching trips:', error);
  ...
  const handleSelectClient = (client) => {
    setIsVisitModalOpen(false); // Close modal if open to prevent confusion
  ...
  const handleSaveVisit = async () => {
    ...
    if (orgError || !orgData) {
        return alert("Error: Could not find Organization.");
    }
    ...
    if (editingVisit) {
        ...
        if (memberError) {
            console.error("Link Error:", memberError);
            alert("Visit created, but some members failed to link.");
        }
    }
  ...
  const handleAddFriendToVisit = async (friend) => {
    if (editingVisit) {
        const { error } = await supabase
            .from('visit_members')
            .insert([{ visit_id: editingVisit.id, client_id: friend.id }]);
  ...
  const displayMembers = editingVisit 
    ? editingVisit.visit_members.map(m => m.client) 
    : pendingFriends; 
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100 rounded-lg overflow-hidden relative">
      ...
      {/* --- TOP SEARCH BAR --- */}
      <div className="bg-white p-4 border-b border-gray-200 shrink-0 z-20">
      ...
      {selectedClient ? (
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-1/3 bg-white border-r border-gray-200 p-6 overflow-y-auto">
                <h2 className="text-lg font-bold text-gray-800 mb-6">Client Profile</h2>
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase">Dives</label>
                            <input 
                                type="number"
                                className="w-full border p-2 rounded mt-1" 
                                value={profileForm.total_dives}
                                onChange={e => setProfileForm({...profileForm, total_dives: e.target.value})}
                            />
                        </div>
                    </div>
                    <button 
                            ...
                            <div key={visit.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm relative group">
                                ...
                                <div className="mb-4">
                                    ...
                                    {myTrips.length > 0 ? (
                                        ...
                                        <div className="text-xs text-gray-400 italic">No trips booked for this visit yet.</div>
                                    )}
                                </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {visit.visit_members?.map(vm => (
                                            <div 
                                                key={vm.client.id} 
                                                onClick={(e) => {
                                                    // NEW: Click to Switch Profile
                                                    e.stopPropagation();
                                                    // We look up the FULL client object from our main list to populate the form correctly
                                                ...
                                                className={`text-xs px-2 py-1 rounded-full border flex items-center gap-1 transition-all ${
                                                    ...
                                                    : 'bg-white border-gray-200 text-gray-600 cursor-pointer hover:border-blue-400 hover:text-blue-600 hover:shadow-sm'
                                                }`}
                                            >
                                                {vm.client.first_name} {vm.client.last_name}
                                                {/* Show small link icon for others */}
        ...
        <div className="flex-1 flex items-center justify-center text-gray-400">
            Use the search bar above to select a client.
        </div>
      )}
